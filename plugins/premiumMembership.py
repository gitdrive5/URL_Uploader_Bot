...
async def broadcast_handler(bot, update):
    try:
        ...
        await update.reply_text(BotMessage.addcommandinvaild, parse_mode = 'Markdown')
    except Exception as e:
        await bot.send_message(Config.OWNER_ID, line_number(fileName, e))
    else:
        if userid.isdigit():
            if addingPremiumUser(userid):
                await update.reply_text(BotMessage.successfullyadded, parse_mode = 'Markdown')
            else:
                await update.reply_text(BotMessage.addingWentWrong, parse_mode = 'Markdown')
        else:
            await update.reply_text(BotMessage.addcommandinvaild, parse_mode = 'Markdown')
