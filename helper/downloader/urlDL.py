...
class URLDL:
    ...
    async def start(self):
        ...
        if len_file == 'Valid':
            msg = await self.bot.edit_message_text(self.userid, self.process_msg_id, BotMessage.starting_to_download, parse_mode = 'Markdown')
            ...
            try:
                ...
                await self.bot.edit_message_text(self.userid, msg.message_id, BotMessage.unsuccessful_upload, parse_mode = 'Markdown')
            else:
                if downObj.isSuccessful():
                    ...
                    n_msg = await self.bot.edit_message_text(self.userid, msg.message_id, BotMessage.uploading_msg, parse_mode = 'Markdown')
                    ...
                    try:
                        ...
                        await self.bot.send_message(self.userid, BotMessage.unsuccessful_upload, parse_mode = 'Markdown')
                    finally:
                        for e in downObj.get_errors():
                            await self.bot.send_message(Config.OWNER_ID, line_number(fileName, e))
        elif len_file == 'Telegram Limit':
            await self.bot.edit_message_text(self.userid, self.process_msg_id, BotMessage.telegramLimit, parse_mode = 'Markdown')
        elif len_file == 'Not Valid':
            await self.bot.edit_message_text(self.userid, self.process_msg_id, BotMessage.unsuccessful_upload, parse_mode = 'Markdown')
