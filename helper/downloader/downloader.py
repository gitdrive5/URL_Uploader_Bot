...
class Downloader:
    ...
    async def start(cls, update, url, bot):
        ...
        self.process_msg = await update.reply_text(BotMessage.processing_url, parse_mode = 'Markdown')
        if match('^(https://)?(www.)?youtu(.)?be(.com)?/(.*)', url):
            await update.reply_text(BotMessage.youtube_url, parse_mode = 'Markdown')
    ...
    async def pdiskRawLinkExtract(self):
        ...
        try:
            ...
            await self.bot.edit_message_text(self.userid, self.process_msg.message_id, BotMessage.unsuccessful_upload, parse_mode = 'Markdown')
            return
        else:
            try:
